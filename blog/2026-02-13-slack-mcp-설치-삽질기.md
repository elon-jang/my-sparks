...
**SSE 방식은 Claude Code CLI에서 작동하지 않는다.** `dynamic client registration`을 지원하지 않기 때문. 이 방식은 Claude Desktop (앱) 전용인 것으로 보인다.

결론: stdio로 복원. 채널 접근이 필요하면 `/invite @Claude-Notification`으로 Bot을 초대하는 게 현실적인 방법이다.
...
5. **SSE는 CLI에서 안 된다**: `dynamic client registration` 미지원 에러 — Claude Desktop 전용
6. **stdio가 현실적 선택**: Bot 초대가 필요하지만, 안정적이고 빠르다
7. **Private 채널은 추가 scope 필요**: `groups:history`, `groups:read` + 앱 재설치
